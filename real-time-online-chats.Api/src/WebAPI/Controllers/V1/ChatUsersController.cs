...
public class ChatUsersController(IChatUserService chatUserService, IHubContext<MessageHub, IMessageClient> messageHub) : AuthorizeController
{
    ...
    public async Task<IActionResult> DeleteMe([FromRoute] Guid chatId, [FromRoute] Guid memberId)
    {
    ...
    {
        var kickResult = await chatUserService.DeleteUser(chatId, memberId, UserId);
